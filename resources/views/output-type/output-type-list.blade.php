                            ...
                            <div class="table-responsive mb-20">
                                <table class="table table-striped table-success">
                                    ...
                                    @foreach($output_types as $output_type)
                                        <tr>
                                            <td class="text-center border-right">{{ $output_type->id }}</td>
                                            <td>{{ $output_type->output_code }}</td>
                                            <td>{{ $output_type->output_name }}</td>
                                            <td class="text-center">
                                                <a href="{{url('output-types/' . $output_type->id .'/edit')}}"
                                                   class="btn btn-primary btn-xs" data-toggle="tooltip"
                                                   data-placement="top" data-original-title="Ubah">
                                                    <i class="fa fa-pencil"></i>
                                                </a>
                                                <a href="#" class="modal_delete btn btn-danger btn-xs"
                                                   data-id="{{$output_type->id}}" data-placement="top"
